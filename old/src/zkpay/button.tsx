...
import { useZkPay } from './hook.js';
import { deriveSecretAsync } from './privy.js';
import { computeCommitment, computeNullifier, bytesToHex } from './crypto.js';
import { generateBurner } from './burner.js';
import type { BurnerWallet } from './burner.js';
import { generateProof } from './circuit.js';
import { setConnected, setDisconnected, setBalance } from './state.js';
import { submitDeposit, submitAuth } from './contract.js';
...
export function ZkPayButton() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setConnected(user, secret, commitment);
        addLog(`Commitment: ${bytesToHex(commitment).slice(0, 20)}…`);
      } catch (err) {
  ...
  const handleDeposit = async () => {
    ...
    setStatusMsg(`Send exactly ${amount} XLM to:`);
    setProgress(25);
        ...
        if (data._embedded?.records?.length > 0) {
          ...
          if (recvAmount >= amount) {
            ...
            try {
              ...
              const txResult = await submitDeposit(zkPay.commitment!, stroops);
              addLog(`Deposit tx: ${txResult.hash}`);
              ...
              setStatusMsg('Deposit recorded locally');
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (!zkPay.secret || !zkPay.commitment) throw new Error('Not connected');
      const nullifierBytes = await computeNullifier(zkPay.commitment, zkPay.secret);

      addLog('Generating proof (may take 30s)…');
      setProgress(50);
      setStatusMsg('Proving with UltraHonk…');

      const result = await generateProof(circuitJson, zkPay.commitment, nullifierBytes, zkPay.secret, (msg) => {
        addLog(msg);
      });
      ...
      const txResult = await submitAuth(result.proof, result.publicInputs, recipientAddress);
      addLog(`Auth tx: ${txResult.hash}`);
                  ...
                  {burner && (
                    <div className="zkpay-burner-section">
                      ...
                      <p className="zkpay-burner-desc">
                        Send exactly <strong>{depositAmount} XLM</strong> to the address below.
